        ...
        <ResponsiveContainer width="100%" height={chartSize.height}>
          <AreaChart data={data} margin={{ top: 20, right: chartSize.rightMargin, left: chartSize.leftMargin, bottom: 10 }}>
            <defs>
              ...
              <filter id="glow-students">
                <feDropShadow dx="0" dy="2" stdDeviation="4" floodColor="#7C3AED" floodOpacity="0.3"/>
              </filter>
              <filter id="glow-courses">
                <feDropShadow dx="0" dy="2" stdDeviation="4" floodColor="#06D6A0" floodOpacity="0.3"/>
              </filter>
            </defs>
            <CartesianGrid strokeDasharray="4 4" stroke="var(--border-color)" strokeOpacity={0.5} />
            <XAxis dataKey="year" tick={{ fill: 'var(--text-secondary)', fontSize: chartSize.tickSize, fontWeight: 700 }} axisLine={{ stroke: 'var(--border-color)' }} />
            <YAxis yAxisId="students" tick={{ fill: 'var(--text-secondary)', fontSize: chartSize.tickSize }} axisLine={false} tickLine={false} />
            <YAxis yAxisId="courses" orientation="right" tick={{ fill: 'var(--text-secondary)', fontSize: chartSize.tickSize }} axisLine={false} tickLine={false} />
            <Tooltip
              contentStyle={{
            ...
            />
            <Area yAxisId="students" type="monotone" dataKey="Students" stroke="#7C3AED" strokeWidth={3} fill="url(#studentsGrad)" dot={{ r: chartSize.dotR, fill: '#7C3AED', stroke: '#fff', strokeWidth: 2 }} activeDot={{ r: chartSize.dotR + 3, fill: '#7C3AED', stroke: '#fff', strokeWidth: 2 }} animationDuration={1500} />
            <Area yAxisId="courses" type="monotone" dataKey="Courses" stroke="#06D6A0" strokeWidth={3} fill="url(#coursesGrad)" dot={{ r: chartSize.dotR, fill: '#06D6A0', stroke: '#fff', strokeWidth: 2 }} activeDot={{ r: chartSize.dotR + 3, fill: '#06D6A0', stroke: '#fff', strokeWidth: 2 }} animationDuration={1500} />
          </AreaChart>
        </ResponsiveContainer>
        <div className="growth-chart-legend">
          <div style={{ textAlign: 'center' }}>
            <div style={{ fontSize: '1.4rem', fontWeight: 800, color: '#7C3AED' }}>4K → 10K</div>
            <div style={{ fontSize: '0.8rem', color: 'var(--text-secondary)' }}>Students</div>
          </div>
          <div style={{ width: 1, background: 'var(--border-color)' }} />
          <div style={{ textAlign: 'center' }}>
            <div style={{ fontSize: '1.4rem', fontWeight: 800, color: '#06D6A0' }}>6 → 20+</div>
      ...
      <section className="section">
        <div className="container">
          ...
          <div className="founders-grid">
            {[
              {
                image: '/images/founder-of-flexcoder.png',
                name: 'Ranjithkumar Thalluru',
                role: 'Founder, FlexCoders.in',
                desc: 'FlexCoders is a training and consulting company headquartered in Madhapur, Hyderabad, bridging the gap between academic learning and industry expectations through hands-on training, expert mentorship, and placement support.',
                about: 'Ranjithkumar Thalluru is the Founder of FlexCoders, started in June 2025. His journey began with a problem he experienced as a student — smart graduates lacking industry-ready skills. Today, FlexCoders has grown to 35+ employees, offering expert-led courses, Pay After Placement programs, and consulting services to build careers one student at a time.'
              },
            ].map((f, i) => (
              <Reveal key={i}>
                <div className="founder-card">
                  <img src={f.image} alt={f.name} className="founder-image" />
          ...
          <Reveal>
            <div className="mv-card" style={{ padding: 40 }}>
              ...
              <div className="life-grid">
                {[
                  '/images/life-at-flexcoders-4.png',
                  '/images/life-at-flexcoders-3.png',
                  '/images/life-at-flexcoders-1.png',
                  '/images/life-at-flexcoders-2.png',
                 ].map((url, i) => (
        ...
        <div className="container">
          ...
          <div style={{ display: 'flex', justifyContent: 'center', gap: 32, flexWrap: 'wrap' }}>
            {[
              ...
              { name: 'GradSculpt', logo: '/images/partner-gradsculpt.png', desc: 'Empowering professionals with cutting-edge skills and career mentorship for a competitive edge.', accent: '#6c63ff' },
            ].map((p, i) => (
              <Reveal key={i}>
